...
import { createPresentation, createSDJWTRelease, createSDJWTwithRelease } from "../src/presentation";


import { issueSDJWTinCombinedFormat } from "../src/issue";
import { HolderBinding } from "../src/types";
...
let TEST_DISCLOSURES: Array<string>;

beforeEach(async () => {
  ({ ISSUER, HOLDER } = await importKeyPairForIssuerAndHolder());
  const sdJWT = await issueSDJWTinCombinedFormat(PAYLOAD, ISSUER.PRIVATE_KEY, HOLDER.PUBLIC_KEY);
  TEST_SD_JWT = sdJWT.split('~')[0];
  TEST_DISCLOSURES = sdJWT.split('~').slice(1);
});


describe('Create Presentation', () => {
  it('create Presentation', async () => {
    // TODO support empty disclosures
    const presentation: string = await createPresentation(TEST_SD_JWT, TEST_DISCLOSURES.slice(-2));
    const splittedPresentation = presentation.split('~');
    expect(splittedPresentation.length).toBe(4);
  });
  it('create Presentation with Holder binding JWT', async () => {
    const holderBinding: HolderBinding = {
      holderKey: HOLDER.PRIVATE_KEY,
      claims: {
    ...
    // TODO support empty disclosures
    const presentation: string = await createPresentation(
      TEST_SD_JWT,
      TEST_DISCLOSURES.slice(-2),
      holderBinding
    );
    const splittedPresentation = presentation.split('~');
    expect(splittedPresentation.length).toBe(4);

    const holderBindingJWT = splittedPresentation[3];
    const JWTPayload = await jwtVerify(holderBindingJWT, HOLDER.PUBLIC_KEY);
    expect(JWTPayload.payload.aud).toBe(holderBinding.claims.aud);
    expect(JWTPayload.payload.nonce).toBe(holderBinding.claims.nonce);
  });
